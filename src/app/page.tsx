  ...
  const handleFileChange = async (
    ...
    reader.onload = async () => {
      ...
      const response = await fetch(
        "https://abhayjha800--audio-cnn-inference-audioclassifier-inference.modal.run/",
        {
          method: "POST",
      ...
      <div className="mx-auto max-w-[60%]">
        <div className="mb-12 text-center">
          ...
          <p className="text-md mb-8 text-stone-600">
            Uploas a WAV file to see the model predictions and feature maps
          </p>
          <div className="flex flex-col items-center">
            <div className="relative inline-block">
              ...
              />
              <Button disabled = {isLoading} className="border-stone-300" variant="outline" size="lg">
                {isLoading ? "Analysing..." : "Choose file"}
              </Button>
            </div>
            {fileName && (<Badge variant={"secondary"} className="mt-4 bg-stone-200 text-stone-600">{fileName}</Badge>)}
          </div>
        </div>
        {error && (
